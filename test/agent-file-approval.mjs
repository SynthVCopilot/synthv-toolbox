...
const ui = readFileSync(new URL("../src/PiDesktop.Tauri/src/main.ts", import.meta.url), "utf8");
...
assert.match(ui, /api\.agentFileApprovals\(\)/);
assert.match(ui, /data-approve-file/);
assert.match(ui, /data-deny-file/);
assert.match(ui, /\[conversations, fileApprovals\] = await Promise\.all/);
