...
assert.match(types, /export interface HttpApiStatus[\s\S]*enabled: boolean;[\s\S]*running: boolean;[\s\S]*port: number;[\s\S]*endpoint: string \| null;[\s\S]*lastError: string \| null;/);
assert.match(api, /getHttpApiStatus: \(\) => call<HttpApiStatus>\("get_http_api_status"\)/);
assert.match(api, /configureHttpApi: \(enabled: boolean, port: number\) =>[\s\S]*call<HttpApiStatus>\("configure_http_api", \{ enabled, port \}\)/);
...
assert.match(main, /configureHttpApi\(enabled, port\)/);
