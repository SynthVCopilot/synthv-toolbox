...
const probeTests = read(join(repositoryRoot, "test", "sv2_account_probe_tests.rs"));
...
assert.match(probe, /equivalent_session_aliases\[request_index\]/);
assert.match(probe, /path = "\.\.\/\.\.\/\.\.\/\.\.\/test\/sv2_account_probe_tests\.rs"/);
assert.doesNotMatch(probe, /mod tests \{/);
assert.match(probeTests, /active_session_reuses_only_fresh_cached_authorization/);
...
assert.match(main, /t\("accountUi.onlyAllowlistedFilesSuchAsSettingsAndScriptsAre"\)/);
assert.match(read(join(webRoot, "i18nAccounts.ts")), /声库按账号独立保存，同账号实例共用槽位数据/);
