...
const release = "https://github.com/lsy-404/platform-kit/releases/download/v0.5.3";
const manifest = JSON.parse(await readFile(path.join(packageRoot, "package.json"), "utf8"));
for (const name of ["core", "providers"]) {
  assert.equal(
    manifest.dependencies[`@model-auth/${name}`],
    `${release}/model-auth-${name}-0.5.3.tgz`,
  );
}
assert.equal(manifest.dependencies["@model-auth/vue"], "https://github.com/lsy-404/platform-kit/releases/download/v0.5.3/model-auth-vue-0.5.3.tgz");
