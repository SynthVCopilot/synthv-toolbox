...
fn fixture_credentials() -> SessionCredentials {
    ...
    parse_session_plaintext(Zeroizing::new(format!(
        "{access}\n{refresh}\n{}\n{}\n",
...
fn current_refresh_failure_is_visible_without_replacing_existing_quarantine() {
    ...
    let fixture = std::env::temp_dir().join(format!("sv2-refresh-failure-{}", uuid::Uuid::new_v4()));
    let root = ProbeRootKey::AccountEnvironment { slot_id: "refresh-failure".to_string(), concurrent: false };
    let fingerprint = SessionCacheKey { canonical_root: fixture.clone(), session_len: 12, last_write_time: 7 };
    ...
    let current = record_active_refresh_failure(
        &root,
        &fingerprint,
        &credentials,
    ...
    assert_eq!(sync_quarantine_get(&root.quarantine_key()).unwrap().detail, old.detail);
    assert_eq!(cached_view_for_fingerprint(&fingerprint, &root).unwrap().session_status, Sv2SessionInspectionStatus::Offline);
    let request = Sv2AccountProbeRequest::for_account(&fixture, false, 0, false, "refresh-failure", false);
    assert_eq!(finish_batch_results(&[request], vec![Some(current)])[0].session_status, Sv2SessionInspectionStatus::Offline);
