...
一次显式预检必须先收集所有闲置的普通与隔离 session，并按“槽位 + JWT 账号主体”分组；Keycloak `sid` / `session_state` 只代表缓存曾经来自哪次登录，不得让同一槽位同一账号的普通与隔离副本各走一轮预检。每组只允许最新 authority 提交一次 refresh、一轮原生等价的 `enroll_device(false)` 检查和一次授权查询；缓存设备 ID 已被服务端判定失效时，该轮可以按原生语义以空 ID 重试，但所有请求必须保持 `kickout_other_sessions=false`。refresh 成功后，必须在继续等待其他网络响应前把新的 token、access expiry 与写入时间同步到同组其他闲置副本，从而尽量缩小 refresh-token rotation 后的分叉窗口。若登录事件返回新的 `device_id` / `user_id`，则按 SV2 自身更新 session 的语义同步该身份；没有返回身份时保留各副本原有身份，并始终逐份保留 `user_id` 之后的不透明缓存字段。所有写入都以原文件指纹做 CAS 原子替换；同步竞争失败或 refresh 响应无法安全验证时，必须使用独立于文件指纹和普通缓存淘汰的槽位级 `SyncFailed` 隔离，使该槽位的普通与并发模式在当前 Toolbox 进程内一起从路由排除。session 瞬时缺失不得解除隔离；只有后续显式预检完整成功、所有可用副本重新收敛后才可清除。不同槽位不得跨写；同槽位出现不同账号主体时，以可读取的普通根为 authority，隔离标记其他主体，不发第二轮预检也不静默覆盖。主体字段缺失、正在运行或指纹已变化的副本不得互拷。这样既避免 refresh-token rotation 令普通/隔离副本互相失效，也不会把已经漂移成其他账号的隔离副本错误覆盖。
