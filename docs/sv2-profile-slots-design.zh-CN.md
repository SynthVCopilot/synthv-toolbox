...
- 本地登录资料探测状态；凭证格式不透明时显示“身份未知”，不要求手工填写用户名或邮箱；
- 自动读取的声库目录证据、试用目录证据和不透明安装项计数；这些信息与账号授权分开显示；
- 自定义颜色或头像首字母；
...
账号资料由工具箱在每次刷新时从受信槽位目录只读探测。当前 `license/session` 是不透明二进制，WebView2 也没有可安全读取的身份字段，因此用户名与邮箱保持未知，不从 Cookie、缓存文本或目录名猜测。公开声库目录可以展示“可下载/可试用”证据，不得把它当作账号授权。密码、原始 Cookie、令牌与 session 内容不会进入前端、日志或 MCP。
...
- 账号卡片内区分“普通启动”和“隔离启动”；
- 自动探测槽位本地登录资料；无法安全解析时显示身份未知，不提供用户名、邮箱或声库授权手填入口；
