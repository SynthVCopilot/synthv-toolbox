...
| 029 | [Feature]_开机启动与托盘 | 实现登录后台启动、状态切换和托盘窗口控制 | 让用户可从设置启用后台常驻并通过托盘打开窗口 | ✅ 已完成 |
| 030 | [BugFix]_托盘图标打包 | 将窗口与托盘图标复制为 Electron 运行时资源并验证构建后路径 | 避免安装版找不到仅在构建资源目录中的图标 | ✅ 已完成 |
| 031 | [BugFix]_本地服务权限布局 | 重组本地 MCP 服务的端点与特权开关布局 | 防止权限说明与控制项在窄窗口中挤压 | ✅ 已完成 |
| 032 | [Feature]_Kit Fluent选择器 | 接入 Platform Kit Fluent 的 Vue Select 组件 | 统一设置页的下拉交互与共享设计系统 | ✅ 已完成 |
