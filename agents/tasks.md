...
| 032 | [Refactor] models.dev运行时目录 | 让模型连接向导和运行时模型资格继续以 models.dev 为配置数据源，并保留已实现协议的本地能力约束与离线回退。 | 上次认证 UI 重构后 models.dev 仅供独立 OpenCode 目录使用，四家运行时提供商重新依赖静态模型数组。 | 🔄 进行中 |
