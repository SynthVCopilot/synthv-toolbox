...
| 034 | [BugFix] concurrent-slots | 修复并发启动、统一账号数据、探测授权和实例跟踪。 | 支持同账号多进程及主槽切换，避免水印声库混用。 | ✅ 已完成 |
| 035 | [Integration] shared-model-auth | 替换共享认证组件，接通原生宿主动作、取消、重连和策略。 | 统一多项目模型接入。 | ✅ 代码与本地验证完成 |
| 036 | [BugFix] session-and-instance-controls | 修复客户端会话误判并增加简洁实例操作，记录令牌接口限制与误报调查。 | 用户验收仍失败且安装程序被错误识别。 | ✅ 已完成 |
| 037 | [BugFix] expired-session-refresh | 过期后先续期，再读取授权。 | 活动账号过期分支直接返回且旧错误可能遮盖真实结果。 | 🔄 进行中 |
