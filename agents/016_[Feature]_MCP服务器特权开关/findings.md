...
- MCP 工具输入不包含权限字段；`toolbox_internal` 与 `toolbox_advanced` 由 Rust 分别固定映射到对应权限域。
- HTTP 状态对前端统一序列化为 `internalFunctionsEnabled` 与 `advancedFunctionsEnabled`，设置存储字段保持 HTTP MCP 命名空间。
- 原有插件 Runtime 与新增 MCP 入口共用一个特权能力执行函数，旧的重复分支已删除。
