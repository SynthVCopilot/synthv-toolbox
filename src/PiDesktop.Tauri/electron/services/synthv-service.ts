...
export class SynthVService {
  ...
  async prepareConcurrentProfile(slotId: string): Promise<Record<string, unknown>> { const store = await this.readStore(); this.slot(store, slotId); const sandbox = await this.sandboxie(); if (!sandbox) return this.profileState(); const box = `SynthVToolbox-${slotId.replaceAll("-", "")}`; const root = join(this.vaultPath(), "instances", slotId.replaceAll("-", "")); await mkdir(root, { recursive: true }); for (const [setting, value] of [["Enabled", "y"], ["FileRootPath", root], ["AutoRecover", "n"], ["NeverDelete", "y"]]) { const result = await this.run(sandbox.ini, ["set", box, setting, value]); if (result.code !== 0) throw new Error(`Sandboxie configuration failed for ${setting}: ${result.stderr}`); } const reload = await this.run(sandbox.start, ["/silent", "/reload"]); if (reload.code !== 0) throw new Error(`Sandboxie configuration reload failed: ${reload.stderr}`); await mkdir(join(this.slotPath(slotId), "sandbox"), { recursive: true }); await writeFile(join(this.slotPath(slotId), "sandbox", "configured"), box, "utf8"); return this.profileState(); }
  ...
  async launchConcurrentProfile(slotId: string, projectPath?: string): Promise<OperationResult> { await this.prepareConcurrentProfile(slotId); const sandbox = await this.sandboxie(); if (!sandbox) return unavailable("Sandboxie Start.exe and SbieIni.exe were not found."); const executable = (await this.scanInstallations()).find(item => item.bridgeProfile === "sv2")?.executablePath; if (typeof executable !== "string") return unavailable("No SynthV Studio 2 executable was found."); const box = `SynthVToolbox-${slotId.replaceAll("-", "")}`; const configured = await exists(join(this.slotPath(slotId), "sandbox", "configured")); if (!configured) return unavailable(`Sandboxie box ${box} has not been configured for this slot.`); const result = await this.run(sandbox.start, [`/box:${box}`, "/silent", executable, ...(projectPath ? [projectPath] : [])]); return result.code === 0 ? ok("SynthV profile launched in Sandboxie.") : fail("Sandboxie failed to launch SynthV.", result.stderr); }
