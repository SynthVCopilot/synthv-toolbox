...
export function registerComponentAudioCommands(registry: CommandRegistry, options: ComponentAudioOptions): void {
  ...
  registry.register("list_synthv_capture_targets", async () => []);
  registry.register("open_ffmpeg_download_page", async () => unavailable("open_ffmpeg_download_page"));
  for (const command of ["plan_audio_prepare", "start_audio_prepare", "analyze_loudness", "plan_loudness_normalize", "start_loudness_normalize", "cancel_audio_job", "audio_job_snapshot", "audio_artifact_info", "save_audio_artifact", "reveal_audio_artifact", "capture_synthv_clip", "compare_synthv_clips", "component_downloads", "queue_component_install", "cancel_component_install", "retry_component_install", "open_downloaded_component", "remove_local_component"]) registry.register(command, async () => unavailable(command));
}
