...
function renderDownload(options: AboutPageOptions): string {
  ...
  if (state.status === "ready") {
    ...
    return `<div class="about-download-state ready"><div><strong>${t("about.readyToInstall")}</strong>${fileName ? `<small>${fileName}</small>` : ""}${macos ? `<small>${t("about.macInstallGuide")}</small>` : ""}</div><button class="primary" data-install-toolbox-update ${options.busy ? "disabled" : ""}>${icon("download", 16)} ${macos ? t("about.openDiskImage") : t("about.installWindows")}</button></div>`;
  }
...
export function renderAboutPage(options: AboutPageOptions): string {
  ...
  const updateResult = update ? `<section class="about-release-result ${update.updateAvailable ? "available" : "current"}"><div class="about-update-status"><span class="feature-icon ${update.updateAvailable ? "orange" : "emerald"}">${icon(update.updateAvailable ? "download" : "check", 22)}</span><div><span class="availability ${update.updateAvailable ? "warning" : "ready"}">${escapeHtml(status)}</span><h3>${escapeHtml(update.releaseName)}</h3><small>${t("about.published")} ${escapeHtml(published)} · ${t("about.checked")} ${escapeHtml(checked)}</small></div></div><div class="result-dashboard compact"><div><small>${t("about.currentVersion")}</small><strong>v${escapeHtml(update.currentVersion)}</strong></div><div><small>${latestLabel}</small><strong>v${escapeHtml(update.latestVersion)}</strong></div></div><div class="about-notes"><strong>${t("about.releaseNotes")}</strong><span>${t("about.officialRelease")}</span><pre class="update-release-notes">${escapeHtml(update.releaseNotes.trim() || t("about.noNotes"))}</pre></div>${renderDownload(options)}</section>` : renderDownload(options);
  const updateAssetActive = options.download?.status === "downloading" || options.download?.status === "ready";
  return `<div class="about-layout"><section class="panel about-product"><div class="about-product-mark"><img class="brand-logo" src="/assets/synthv-toolbox-logo.svg" alt="Synthesizer V Toolbox" /></div><div class="about-product-copy"><span class="eyebrow">SYNTHESIZER V TOOLBOX</span><h2>Synthesizer V Toolbox</h2><p>${t("about.productDescription")}</p><code class="about-project-url">github.com/SynthVCopilot/synthv-toolbox</code></div><dl class="about-product-details"><div><dt>${t("about.build")}</dt><dd>v${escapeHtml(app.appVersion)}</dd></div><div><dt>${t("about.platform")}</dt><dd>${escapeHtml(app.platform)}</dd></div><div><dt>${t("about.license")}</dt><dd>Apache-2.0</dd></div></dl><div class="about-links"><button class="secondary" data-open-toolbox-project="project">${icon("github", 16)} ${t("about.project")}</button><button class="secondary" data-open-toolbox-project="guide">${icon("info", 16)} ${t("about.guidance")}</button><button class="secondary" data-open-toolbox-project="issues">${icon("warning", 16)} ${t("about.issues")}</button></div></section><section class="panel about-updates"><div class="section-heading"><div><h2>${t("about.updates")}</h2><p>${t("about.updatesDescription")}</p></div></div><div class="about-update-controls"><label class="field"><span>${t("about.channel")}</span><select class="fluent-select" id="update-channel" ${options.busy || updateAssetActive ? "disabled" : ""}><option value="stable" ${app.updateChannel === "stable" ? "selected" : ""}>${t("about.stable")}</option><option value="nightly" ${app.updateChannel === "nightly" ? "selected" : ""}>${t("about.nightly")}</option></select><small>${app.updateChannel === "nightly" ? t("about.nightlyDescription") : t("about.stableDescription")}</small></label><div class="about-check-action"><div><small>${t("about.currentVersion")}</small><strong>v${escapeHtml(app.appVersion)}</strong></div><button class="secondary" data-check-toolbox-update ${options.busy || updateAssetActive ? "disabled" : ""}>${icon("sync", 16)} ${update ? t("about.checkAgain") : t("about.check")}</button></div></div>${updateResult}</section></div>`;
}
