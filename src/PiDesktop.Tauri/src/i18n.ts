...
const messages = {
  "zh-CN": {
    ...
    settings: { language: "显示语言", chinese: "中文", english: "English", mode: "运行模式", modeDescription: "切换后导航与 Rust 后端能力会同时更新。", toolbox: "纯工具箱", toolboxDescription: "确定性基础流程，不启动 AI", ai: "AI 模式", aiDescription: "Copilot、智能增强与 MCP", aiDisabled: "AI 运行时已关闭", aiDisabledDescription: "当前不会显示 Copilot、模型或 MCP 设置，也不会向模型端点发送请求。", providers: "模型提供商", providersDescription: "可使用 OAuth 订阅或多份 API Key 接入；凭据均由本机后端处理。", addProvider: "添加或切换连接", noProvider: "尚未选择提供商", chooseProvider: "选择认证方式、提供商与模型后即可开始对话。", smartRoute: "智能 .svp 启动", smartRouteDescription: "根据工程所需声库，从空闲账号中建议最合适的启动槽位。", enabled: "已开启", disabled: "已关闭", openDefaults: "打开默认应用设置", update: "应用更新", updateDescription: "按需检查官方 GitHub Releases；不会自动下载或安装。", currentVersion: "当前版本", checkUpdate: "检查更新", checkAgain: "重新检查", dataPlatform: "数据与平台", dataPlatformDescription: "配置和历史使用统一的跨平台用户目录。", platform: "平台", config: "配置", appVersion: "应用版本" },
