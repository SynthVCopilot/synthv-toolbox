...
const messages = {
  "zh-CN": {
    nav: { workspace: "工作区", system: "系统", home: "概览", accounts: "SV2 账号", lyrics: "作词", history: "历史", copilot: "Copilot", ai: "AI", components: "组件中心", bridge: "SynthV Bridge", connections: "连接", settings: "设置", expand: "展开侧栏", collapse: "收起侧栏" },
    pages: { home: ["概览", "查看环境状态与常用能力"], accounts: ["SV2 账号", "管理本机 SV2 槽位；Windows 还支持可选并发隔离"], import: ["导入", "从一个来源开始完成 Cover 工作流"], convert: ["转换", "处理音频、提取旋律，或准备可继续编辑的素材"], analysis: ["分析", "了解人声表现与音频结构，为下一步创作提供依据"], quality: ["质检", "检查工程、发音和渲染结果，定位需要处理的问题"], lyrics: ["作词", "专注写下歌词，需要时再调用结构、韵脚与 AI 辅助"], history: ["历史", "查看工作流记录与工程自动备份"], copilot: ["Copilot", "让 AI 在受控工具边界内协助工作"], ai: ["AI", "管理模型连接并查看订阅使用详情"], components: ["组件中心", "管理本地模型与运行组件"], bridge: ["SynthV Bridge", "探测、安装、诊断并连接 Synthesizer V"], connections: ["连接", "管理本机服务和外部 MCP 工具"], settings: ["设置", "调整运行模式与模型配置"] },
    settings: { language: "显示语言", chinese: "中文", english: "English", mode: "运行模式", modeDescription: "切换后导航与 Rust 后端能力会同时更新。", toolbox: "纯工具箱", toolboxDescription: "确定性基础流程，不启动 AI", ai: "AI 模式", aiDescription: "Copilot、智能增强与 MCP", aiDisabled: "AI 运行时已关闭", aiDisabledDescription: "当前不会显示 Copilot、模型或外部 MCP 设置，也不会向模型端点发送请求。工具箱自身的本地 MCP 服务仍可在连接页使用。", providers: "模型提供商", providersDescription: "可使用 OAuth 订阅或多份 API Key 接入；凭据均由本机后端处理。", addProvider: "添加或切换连接", noProvider: "尚未选择提供商", chooseProvider: "选择认证方式、提供商与模型后即可开始对话。", notConfigured: "未配置连接", legacyCredentials: "检测到旧版 API token 配置", legacyCredentialsDescription: "旧配置不会作为 OAuth 账号展示。请完成浏览器授权；后端迁移完成前仍会保留旧配置。", oauthSummary: "{count} 个 OAuth · {keys} 个 API Key", smartRoute: "智能 .svp 启动", smartRouteDescription: "根据工程格式和已安装宿主，选择合适的 SynthV 启动方式。", enabled: "已开启", disabled: "已关闭", unsupported: "当前平台不支持", defaultApp: "已设为 .svp 默认打开方式", registered: "已注册，等待设为默认应用", unregistered: "尚未注册为可选打开方式", openDefaults: "打开默认应用设置", update: "应用更新", updateDescription: "按需检查官方 GitHub Releases；不会自动下载或安装。", currentVersion: "当前版本", checkUpdate: "检查更新", checkAgain: "重新检查", dataPlatform: "数据与平台", dataPlatformDescription: "配置和历史使用统一的跨平台用户目录。", platform: "平台", config: "配置", appVersion: "应用版本" },
...
const chineseConnections = messages["zh-CN"].connections as Record<string, string>;
...
settingsMessages.updateChannel = "更新渠道";
settingsMessages.stable = "稳定版";
settingsMessages.nightly = "Nightly";
settingsMessages.stableDescription = "仅检查正式发布版本。";
settingsMessages.nightlyDescription = "检查最新开发构建；它可能不稳定。";
settingsMessages.latestNightlyVersion = "最新 Nightly 版本";
chineseConnections.internalFunctions = "允许内部函数";
chineseConnections.internalFunctionsDescription = "允许 MCP 调用尚未完整包装为公开 API 的宿主功能。";
chineseConnections.advancedFunctions = "允许高级功能";
chineseConnections.advancedFunctionsDescription = "允许沙箱增删、授权信息修改、不限制网络请求，以及插件数据目录外的读写。";
chineseConnections.privilegesRequireMcp = "这些特权设置会被保存，但仅在 MCP 工具接口开启时生效。";
chineseConnections.endpointAccess = "接口访问";
chineseConnections.endpointAccessDescription = "分别开启 MCP 工具和 Agent 对话接口。";
chineseConnections.privilegedAccess = "特权访问";
...
const englishConnections = messages.en.connections as Record<string, string>;
...
englishSettingsMessages.updateChannel = "Update channel";
englishSettingsMessages.stable = "Stable";
englishSettingsMessages.nightly = "Nightly";
englishSettingsMessages.stableDescription = "Check released versions only.";
englishSettingsMessages.nightlyDescription = "Check the latest development build; it may be unstable.";
englishSettingsMessages.latestNightlyVersion = "Latest nightly version";
englishConnections.internalFunctions = "Allow internal functions";
englishConnections.internalFunctionsDescription = "Allow MCP to call host functions that are not yet fully wrapped as public APIs.";
englishConnections.advancedFunctions = "Allow advanced functions";
englishConnections.advancedFunctionsDescription = "Allow sandbox creation and removal, authorization changes, unrestricted network requests, and reading or writing outside plugin data directories.";
englishConnections.privilegesRequireMcp = "These privilege settings are saved, but take effect only when the MCP tools endpoint is enabled.";
englishConnections.endpointAccess = "Endpoint access";
englishConnections.endpointAccessDescription = "Enable MCP tools and Agent chat independently.";
englishConnections.privilegedAccess = "Privileged access";
...
addMessages("zh-CN", { settings: { alwaysAsk: "始终询问启动方式", chooseSynthVHost: "选择 SynthV 打开方式", svpFormatAmbiguous: "工程格式需要确认", hostInstallation: "本机安装", openWithHost: "使用此方式打开", noCompatibleHosts: "没有检测到可用于此工程的 SynthV 安装。" } });
addMessages("en", { settings: { alwaysAsk: "Always ask how to open", chooseSynthVHost: "Choose a SynthV host", svpFormatAmbiguous: "Project format needs confirmation", hostInstallation: "Installed locally", openWithHost: "Open with this host", noCompatibleHosts: "No SynthV installations available for this project were detected." } });
