...
let previewLyricBridgeGeneration = 0;
let previewLyricBridgePreviewToken: string | undefined;
...
async function call<T>(command: string, args?: Record<string, unknown>): Promise<T> {
  ...
  if (command === "check_toolbox_update") {
    const previewNightly = previewState().updateChannel === "nightly";
    const previewVersion = previewNightly ? "0.3.0-dev.abcdef0" : "0.2.0";
    const previewTag = previewNightly ? "v0.3.0-nightly" : "v0.2.0";
    const previewInstaller = previewNightly ? "SynthV.Toolbox_0.3.0_dev.abcdef0_x64-setup.exe" : "SynthV.Toolbox_0.2.0_x64-setup.exe";
    return {
      channel: previewState().updateChannel,
      currentVersion: previewState().appVersion,
      latestVersion: previewVersion,
      updateAvailable: true,
      releaseName: previewNightly ? "SynthV Toolbox v0.3.0 Pre-release" : "Synthesizer V Toolbox v0.2.0",
      releaseUrl: `https://github.com/SynthVCopilot/synthv-toolbox/releases/tag/${previewTag}`,
      publishedAtUtc: new Date().toISOString(),
      releaseNotes: "## 更新内容\n\n- 新增更新检查工具\n- 修复若干问题",
      checkedAtUtc: new Date().toISOString(),
      installer: { name: previewInstaller, url: `https://github.com/SynthVCopilot/synthv-toolbox/releases/download/${previewTag}/${previewInstaller}`, sha256: "0".repeat(64), size: 1 },
    } as T;
  }
  ...
  if (command === "open_toolbox_releases" || command === "open_toolbox_project") return {
    succeeded: true,
    summary: "已打开 Synthesizer V Toolbox 发布页。",
  ...
  if (command === "read_lyric_bridge_selection") return {
    selectionToken: `preview-selection-${++previewLyricBridgeGeneration}`, sessionToken: "preview-session", target: { trackIndex: 1, groupIndex: 1 },
    notes: [{ noteIndex: 1, lyric: "la" }, { noteIndex: 2, lyric: "la" }], noteCount: 2,
  } as T;
  if (command === "preview_lyric_bridge_fit") {
    ...
    if (request?.selectionToken !== `preview-selection-${previewLyricBridgeGeneration}` || request?.sessionToken !== "preview-session" || request.slots?.length !== 2 || request.slots.some((slot) => !slot.text?.trim())) throw new Error("预览词槽无效。");
    previewLyricBridgePreviewToken = `preview-write-${previewLyricBridgeGeneration}`;
    return { previewToken: previewLyricBridgePreviewToken, sessionToken: "preview-session", target: { trackIndex: 1, groupIndex: 1 }, notes: request.slots.map((slot, index) => ({ noteIndex: index + 1, currentLyric: "la", text: slot.text!.trim() })), slotCount: 2, expiresInSeconds: 60 } as T;
  }
  if (command === "confirm_lyric_bridge_fit") {
    if ((args?.request as { previewToken?: string } | undefined)?.previewToken !== previewLyricBridgePreviewToken) throw new Error("预览已失效。");
    previewLyricBridgePreviewToken = undefined;
    return { applied: true, target: { trackIndex: 1, groupIndex: 1 }, noteCount: 2 } as T;
  }
...
export const api = {
  ...
  getToolboxUpdateDownload: () => call<import("./types").ToolboxUpdateDownload>("get_toolbox_update_download"),
  downloadToolboxUpdate: () => call<import("./types").ToolboxUpdateDownload>("download_toolbox_update"),
  cancelToolboxUpdateDownload: () => call<import("./types").ToolboxUpdateDownload>("cancel_toolbox_update_download"),
  installToolboxUpdate: () => call<OperationResult>("install_toolbox_update"),
