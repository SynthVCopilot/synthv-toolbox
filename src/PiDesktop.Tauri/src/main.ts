...
function pathPickerButton(inputId: string, kind: "file" | "audio" | "directory"): string {
  ...
  return `<button class="path-picker-button" type="button" data-pick-path="${inputId}" data-pick-path-kind="${kind}" title="${escapeHtml(label)}" aria-label="${escapeHtml(label)}">${icon(kind === "directory" ? "folder" : "file", 16)}</button>`;
}
...
async function pickPathIntoInput(inputId: string, kind: "file" | "audio" | "directory"): Promise<void> {
  ...
  if (!path || page !== selectionPage) return;
  ...
  if (id === "audio-preparation") {
    ...
    form = `<div class="mode-limit"><strong>${t("workflowCopy.conversionScope")}</strong>${t("workflowCopy.writeMonophonicMidiMusicxmlIntoTheOpen")}</div>
      <form id="score-to-synthv-form" class="workflow-form workflow-wide">
        <label>${t("workflowCopy.scoreFilePath")}<div class="path-picker-control"><input id="score-source-path" required placeholder="${t("workflowCopy.localMidMidiXmlMusicxmlOrMxl")}" />${pathPickerButton("score-source-path", "file")}</div></label>
