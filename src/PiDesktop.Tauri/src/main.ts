...
function renderMcp(): string {
  ...
  return `<div class="connections-layout"><section class="panel http-api-settings"><div class="section-heading"><div><h2>${t("connections.localService")}</h2><p>${t("connections.localServiceDescription")}</p></div><span class="availability ${httpApiStatus.running ? "ready" : httpApiStatus.enabled || httpApiStatus.agentEnabled ? "warning" : ""}">${httpApiStatus.running ? t("connections.running") : httpApiStatus.enabled || httpApiStatus.agentEnabled ? t("connections.failed") : t("connections.off")}</span></div><form id="http-api-form" class="http-api-form"><section class="http-api-access-group" aria-labelledby="http-api-endpoint-heading"><div class="http-api-group-heading"><h3 id="http-api-endpoint-heading">${t("connections.endpointAccess")}</h3><p>${t("connections.endpointAccessDescription")}</p></div><div class="http-api-endpoint-grid"><label class="fluent-switch large"><input id="http-api-enabled" name="enabled" type="checkbox" ${httpApiStatus.enabled ? "checked" : ""} aria-label="${t("connections.mcpTools")}" aria-describedby="http-api-help" /><span></span>${t("connections.mcpTools")}</label><label class="fluent-switch large"><input id="http-agent-enabled" name="agentEnabled" type="checkbox" ${httpApiStatus.agentEnabled ? "checked" : ""} aria-label="${t("connections.agentChat")}" aria-describedby="http-api-help" /><span></span>${t("connections.agentChat")}</label></div></section><section class="http-api-access-group" aria-labelledby="http-api-privilege-heading"><div class="http-api-group-heading"><h3 id="http-api-privilege-heading">${t("connections.privilegedAccess")}</h3><p>${t("connections.privilegesRequireMcp")}</p></div><div class="http-api-privilege-grid"><label class="fluent-switch large http-api-privilege"><input id="http-internal-functions-enabled" name="internalFunctionsEnabled" type="checkbox" ${httpApiStatus.internalFunctionsEnabled ? "checked" : ""} aria-label="${t("connections.internalFunctions")}" aria-describedby="http-api-help" /><span></span><span><strong>${t("connections.internalFunctions")}</strong><small>${t("connections.internalFunctionsDescription")}</small></span></label><label class="fluent-switch large http-api-privilege danger"><input id="http-advanced-functions-enabled" name="advancedFunctionsEnabled" type="checkbox" ${httpApiStatus.advancedFunctionsEnabled ? "checked" : ""} aria-label="${t("connections.advancedFunctions")}" aria-describedby="http-api-help" /><span></span><span><strong>${t("connections.advancedFunctions")}</strong><small>${t("connections.advancedFunctionsDescription")}</small></span></label></div></section><div class="http-api-actions"><label class="http-api-port">${t("connections.port")}<input id="http-api-port" name="port" type="number" min="1" max="65535" step="1" value="${httpApiStatus.port || 17831}" inputmode="numeric" required aria-describedby="http-api-help" /></label><button class="primary" type="submit" ${busy ? "disabled" : ""}>${t("connections.apply")}</button></div></form><div id="http-api-help" class="http-api-status"><span><strong>${t("connections.listening")}</strong>${httpApiStatus.running ? t("connections.active") : httpApiStatus.enabled || httpApiStatus.agentEnabled ? t("connections.inactive") : t("connections.disabled")}</span>${httpApiStatus.endpoint ? `<span><strong>MCP</strong><code>${escapeHtml(httpApiStatus.endpoint)}</code></span>` : ""}${httpApiStatus.agentEndpoint ? `<span><strong>Agent</strong><code>${escapeHtml(httpApiStatus.agentEndpoint)}</code></span>` : ""}${httpApiStatus.lastError ? `<span class="error-text"><strong>${t("connections.error")}</strong>${escapeHtml(httpApiStatus.lastError)}</span>` : ""}</div></section>${externalMcp}</div>`;
}
