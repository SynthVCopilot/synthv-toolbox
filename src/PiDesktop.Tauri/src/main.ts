...
function renderWorkflowPanel(id: string): string {
  ...
  if (id === "audio-preparation") {
    ...
    const probeCard = probe
      ? `<section class="audio-probe-card" aria-label="${t('')}"><div class="audio-probe-heading"><div><span class="eyebrow">MEDIA PROBE</span><strong>${escapeHtml(probe.codec ?? "${t('')}")}</strong></div><span class="availability ready">${t('')}</span></div><dl class="audio-metadata"><div><dt>${t('')}</dt><dd>${escapeHtml(probe.container ?? "未知")}</dd></div><div><dt>${t('')}</dt><dd>${formatAudioNumber(probe.durationSeconds, " 秒")}</dd></div><div><dt>${t('')}</dt><dd>${formatAudioNumber(probe.sampleRate, " Hz")}</dd></div><div><dt>${t('')}</dt><dd>${formatAudioNumber(probe.channels)}${probe.channelLayout ? ` · ${escapeHtml(probe.channelLayout)}` : ""}</dd></div><div><dt>${t('')}</dt><dd>${formatAudioNumber(probe.bitDepth, " bit")}</dd></div><div><dt>${t('')}</dt><dd>${formatAudioNumber(probe.bitRate ? probe.bitRate / 1000 : undefined, " kb/s")}</dd></div></dl>${probe.sourceArtifactId && probe.sourceMimeType ? `<div class="audio-source-preview">${audioSourcePreviewUrl ? `<audio controls preload="metadata" src="${escapeHtml(audioSourcePreviewUrl)}" data-audio-preview-artifact="${escapeHtml(probe.sourceArtifactId)}" data-audio-preview-kind="source" data-audio-preview-generation="${audioInputGeneration}" aria-label="原音试听"></audio>` : `<button class="secondary" data-preview-audio-artifact="${escapeHtml(probe.sourceArtifactId)}" data-audio-preview-kind="source" ${audioArtifactActionInFlight ? "disabled" : ""}>${icon("play", 16)} ${t('')}</button>`}</div>` : ""}</section>`
      : `<div class="audio-empty-probe" role="status">选择一个本地音频后，将显示${t('')}、编码、${t('')}、${t('')}、${t('')}、${t('')}与${t('')}。</div>`;
    ...
    const jobPanel = audioJob
      ? `<section class="audio-job-card ${audioJob.status}" aria-live="polite"><div class="audio-job-heading"><div><span class="eyebrow">AUDIO TASK</span><strong>${audioJob.operation === "loudness-normalize" ? "响度标准化" : "PCM WAV 转码"}</strong></div><span class="availability ${audioJob.status === "completed" ? "ready" : audioJob.status === "failed" ? "warning" : ""}">${escapeHtml(audioJob.status)}</span></div>${audioJob.progressPercent !== undefined ? `<div class="audio-progress" role="progressbar" aria-label="音频处理进度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.round(audioJob.progressPercent)}"><span style="width:${Math.max(0, Math.min(100, audioJob.progressPercent))}%"></span></div><small>${formatAudioNumber(audioJob.progressPercent, "%")}</small>` : `<small>${running ? "${t('')}" : "${t('')}"}</small>`}${audioJob.outputPath ? `<div class="audio-output-path"><span>${t('')}</span><code>${escapeHtml(audioJob.outputPath)}</code></div>` : ""}${audioJob.error ? `<pre class="audio-job-error">${escapeHtml(audioJob.error)}</pre>` : ""}${audioJob.loudnessReport ? `<div class="audio-loudness-readout compact"><span>复测 ${formatAudioNumber(audioJob.loudnessReport.integratedLufs, " LUFS")}</span><span>峰值 ${formatAudioNumber(audioJob.loudnessReport.truePeakDbtp, " dBTP")}</span><span>LRA ${formatAudioNumber(audioJob.loudnessReport.loudnessRange, " LU")}</span></div>` : ""}${running ? `<div class="button-row"><button class="secondary" data-cancel-audio-job="${escapeHtml(audioJob.id)}" ${audioCancelInFlight ? "disabled" : ""}>${audioCancelInFlight ? "${t('')}" : "${t('')}"}</button></div>` : audioJob.status === "completed" && audioJob.artifactId ? `<div class="audio-artifact-actions">${audioPreviewUrl ? `<audio controls preload="metadata" src="${escapeHtml(audioPreviewUrl)}" data-audio-preview-artifact="${escapeHtml(audioJob.artifactId)}" data-audio-preview-kind="result" data-audio-preview-generation="${audioInputGeneration}" aria-label="结果试听"></audio>` : `<button class="secondary" data-preview-audio-artifact="${escapeHtml(audioJob.artifactId)}" ${audioArtifactActionInFlight ? "disabled" : ""}>${icon("play", 16)} ${t('')}</button>`}<button class="secondary" data-reveal-audio-artifact="${escapeHtml(audioJob.artifactId)}" ${audioArtifactActionInFlight ? "disabled" : ""}>${t('')}</button><button class="secondary" data-copy-audio-artifact="${escapeHtml(audioJob.artifactId)}" ${audioArtifactActionInFlight ? "disabled" : ""}>${t('')}</button><button class="primary" data-save-audio-artifact="${escapeHtml(audioJob.artifactId)}" ${audioArtifactActionInFlight ? "disabled" : ""}>${t('')}</button></div>` : ""}</section>`
      : "";
    form = `<div class="audio-preparation" aria-label="音频准备">
      <section class="audio-runtime-card ${runtime?.available ? "ready" : "warning"}"><div><span class="eyebrow">FFMPEG RUNTIME</span><strong>${runtime === undefined ? "正在检查…" : runtime.available ? `可用${runtime.version ? ` · ${escapeHtml(runtime.version)}` : ""}` : "未找到 FFmpeg"}</strong><small>${escapeHtml(runtime?.detail ?? "进入此工具后检查本机 FFmpeg。")}${runtime !== undefined && !runtime.available ? " 请在组件中心安装或修复 FFmpeg 后${t('')}。" : ""}</small></div><span class="availability ${runtime?.available ? "ready" : "warning"}">${escapeHtml(runtime?.source ?? (runtime === undefined ? "检查中" : runtime.available ? "已就绪" : "需要组件"))}</span></section>
      <section class="audio-input-card"><div class="section-heading"><div><h3>${t('')}</h3><p>支持点击选择或拖放一个本地文件；不会自动导入 SynthV、运行 CVRS 或批处理。</p></div><button type="button" class="secondary" data-pick-audio-file ${controlsLocked ? "disabled" : ""}>${t('')}</button></div><label class="visually-hidden" for="audio-prep-input">音频文件路径</label><input id="audio-prep-input" value="${escapeHtml(audioPrepareForm.inputPath)}" readonly placeholder="尚未${t('')}" aria-describedby="audio-drop-help" /><button type="button" class="audio-drop-zone" data-audio-drop-zone aria-label="拖放一个音频文件或${t('')}" aria-describedby="audio-drop-help" ${controlsLocked ? "disabled" : ""}><span>${icon("audio", 22)}</span><strong>${t('')}</strong><small id="audio-drop-help">一次只接受一个文件。拖放后先进行只读媒体探测。</small></button></section>
      ${probeCard}
      <div class="audio-action-grid"><section class="audio-action-card"><div><span class="eyebrow">SYNTHV PCM WAV</span><h3>为 SynthV 准备 PCM WAV</h3><p>默认保持原${t('')}和${t('')}，输出 24-bit PCM WAV。</p></div><form id="audio-prepare-form" class="workflow-form"><div class="workflow-pair three"><label>${t('')}（Hz）<input id="audio-prep-rate" type="number" min="8000" max="192000" step="1" value="${audioPrepareForm.sampleRate ?? ""}" placeholder="保持不变" ${controlsLocked ? "disabled" : ""}/></label><label>${t('')}数<select id="audio-prep-channels" ${controlsLocked ? "disabled" : ""}><option value="">保持不变</option><option value="1" ${audioPrepareForm.channels === 1 ? "selected" : ""}>单${t('')}</option><option value="2" ${audioPrepareForm.channels === 2 ? "selected" : ""}>立体声</option></select></label><label>${t('')}<select id="audio-prep-format" ${controlsLocked ? "disabled" : ""}><option value="s16" ${audioPrepareForm.sampleFormat === "s16" ? "selected" : ""}>16-bit PCM</option><option value="s24" ${audioPrepareForm.sampleFormat === "s24" ? "selected" : ""}>24-bit PCM</option><option value="f32" ${audioPrepareForm.sampleFormat === "f32" ? "selected" : ""}>32-bit float</option></select></label></div><div class="workflow-pair"><label>起始位置（秒）<input id="audio-prep-start" type="number" min="0" step="0.01" value="${audioPrepareForm.startSeconds ?? ""}" placeholder="从头开始" ${controlsLocked ? "disabled" : ""}/></label><label>${t('')}（秒）<input id="audio-prep-duration" type="number" min="0.01" step="0.01" value="${audioPrepareForm.durationSeconds ?? ""}" placeholder="直到结尾" ${controlsLocked ? "disabled" : ""}/></label></div><button class="primary" ${!audioPrepareForm.inputPath || !runtime?.available || controlsLocked ? "disabled" : ""}>${icon("audio", 16)} ${t('')}</button></form></section>
      <section class="audio-action-card"><div><span class="eyebrow">EBU R128</span><h3>检查 / 平衡响度</h3><p>默认目标 −16 LUFS / −1.5 dBTP / 11 LRA，写入后会自动复测。</p></div><div class="button-row"><button class="secondary" data-analyze-audio-loudness ${!audioPrepareForm.inputPath || !runtime?.available || controlsLocked ? "disabled" : ""}>${t('')}</button></div>${loudness}<form id="audio-normalize-form" class="workflow-form"><div class="workflow-pair three"><label>LUFS<input id="audio-normalize-lufs" type="number" min="-70" max="-5" step="0.1" required value="${audioNormalizeForm.integratedLufs}" ${controlsLocked ? "disabled" : ""}/></label><label>dBTP<input id="audio-normalize-peak" type="number" min="-9" max="0" step="0.1" required value="${audioNormalizeForm.truePeakDbtp}" ${controlsLocked ? "disabled" : ""}/></label><label>LRA<input id="audio-normalize-lra" type="number" min="1" max="20" step="0.1" required value="${audioNormalizeForm.loudnessRange}" ${controlsLocked ? "disabled" : ""}/></label></div><button class="primary" ${!audioPrepareForm.inputPath || !runtime?.available || controlsLocked ? "disabled" : ""}>${icon("shield", 16)} ${t('')}</button></form></section></div>
    ...
    const taskCards = mediaTasks.filter((item) => item.kind === "cover").slice(-5).reverse().map((task) => {
      ...
      const action = ["queued", "running", "cancelling"].includes(task.status)
        ? `<button class="secondary compact" data-cancel-media-task="${escapeHtml(task.id)}" ${task.status === "cancelling" ? "disabled" : ""}>${task.status === "cancelling" ? "终止中…" : "${t('')}"}</button>`
        : ["failed", "cancelled"].includes(task.status)
          ? `<button class="secondary compact" data-retry-media-task="${escapeHtml(task.id)}">${t('')}</button>`
          : "";
    ...
    const taskList = taskCards ? `<section class="download-queue"><div class="section-heading"><div><h3>Cover 任务</h3><p>下载、分离与旋律提取均可${t('')}；Bridge 写入开始后以宿主实际结果为准。</p></div></div><div class="download-list">${taskCards}</div></section>` : "";
    ...
    const taskList = taskCards ? `<section class="download-queue"><div class="section-heading"><div><h3>媒体任务</h3><p>状态会持久化；${t('')}会终止 yt-dlp 及其帮助进程。</p></div></div><div class="download-list">${taskCards}</div></section>` : "";
    ...
    const taskList = taskCards ? `<section class="download-queue"><div class="section-heading"><div><h3>分离任务</h3><p>状态会持久化；${t('')}会终止 Python、Demucs 及模型帮助进程。</p></div></div><div class="download-list">${taskCards}</div></section>` : "";
    ...
    form = `<form id="audio-probe-form" class="workflow-form">
      ...
      <button class="primary">${icon(ai ? "sparkles" : "play", 16)} ${t('')}</button>
    </form>`;
    ...
    form = `<div class="mode-limit"><strong>转换范围：</strong>把单声部 MIDI / MusicXML 写入当前打开的 SynthV 工程；不会跨版本直译 SV1 / SV2 的歌手、唱法或参数，也不会代替你保存 .svp。</div>
      <form id="score-to-synthv-form" class="workflow-form workflow-wide">
        ...
        <button class="primary" ${app.bridgeConnected ? "" : "disabled"}>${icon("file", 16)} ${app.bridgeConnected ? "转换并导入当前工程" : "${t('')}"}</button>
      </form>`;
    ...
    const targetControl = !audioCaptureCapability
      ...
      : `<div class="mode-limit">没有发现 SynthV standalone 进程。请启动 SynthV 2 Pro 后${t('')}。</div>`;
  ...
  return `<section class="workflow-panel"><div class="workflow-heading"><span class="feature-icon ${feature?.accent ?? "violet"}">${icon(feature?.icon ?? "toolbox", 25)}</span><div><span class="eyebrow">${escapeHtml(group?.title ?? t(''))}</span><h2>${escapeHtml(feature?.title ?? t(''))}</h2><p>${escapeHtml(feature?.description ?? "")}</p></div></div>${form}${result}</section>`;
}
