...
function renderSettings(): string {
  ...
  return `<div class="settings-layout"><section class="panel"><div class="section-heading"><div><h2>运行模式</h2><p>切换后导航与 Rust 后端能力会同时更新。</p></div></div><div class="mode-setting"><button class="setting-choice ${app.mode === "toolbox" ? "active" : ""}" data-set-mode="toolbox"><span class="mode-icon slate">${icon("toolbox", 23)}</span><span><strong>纯工具箱</strong><small>确定性基础流程，不启动 AI</small></span>${app.mode === "toolbox" ? icon("check", 20) : ""}</button><button class="setting-choice ${app.mode === "ai" ? "active" : ""}" data-set-mode="ai"><span class="mode-icon purple">${icon("sparkles", 23)}</span><span><strong>AI 模式</strong><small>Copilot、智能增强与 MCP</small></span>${app.mode === "ai" ? icon("check", 20) : ""}</button></div></section>
    ...
    <section class="panel http-api-settings"><div class="section-heading"><div><h2>本地 HTTP MCP API</h2><p>仅监听本机回环地址，供 OpenCode 等外部程序复用 Toolbox 工具；默认关闭。</p></div><span class="availability ${httpApiStatus.running ? "ready" : httpApiStatus.enabled ? "warning" : ""}">${httpApiStatus.running ? "运行中" : httpApiStatus.enabled ? "启动失败" : "已关闭"}</span></div><form id="http-api-form" class="http-api-form"><label class="fluent-switch large"><input id="http-api-enabled" name="enabled" type="checkbox" ${httpApiStatus.enabled ? "checked" : ""} aria-describedby="http-api-help" /><span></span><span>${httpApiStatus.enabled ? "已开启" : "已关闭"}</span></label><label class="http-api-port">监听端口<input id="http-api-port" name="port" type="number" min="1" max="65535" step="1" value="${httpApiStatus.port || 17831}" inputmode="numeric" required aria-describedby="http-api-help" /></label><button class="primary" type="submit" ${busy ? "disabled" : ""}>应用并保存</button></form><div id="http-api-help" class="http-api-status"><span><strong>状态</strong>${httpApiStatus.running ? "正在监听" : httpApiStatus.enabled ? "未运行" : "未启用"}</span>${httpApiStatus.endpoint ? `<span><strong>端点</strong><code>${escapeHtml(httpApiStatus.endpoint)}</code></span>` : ""}${httpApiStatus.lastError ? `<span class="error-text"><strong>错误</strong>${escapeHtml(httpApiStatus.lastError)}</span>` : ""}</div></section>
