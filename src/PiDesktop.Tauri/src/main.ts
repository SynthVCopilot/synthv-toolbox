...
function renderAccountManager(): string {
  ...
  const tabs = "";
  return `<div class="dialog-backdrop account-manager-backdrop" role="presentation"><section class="account-manager-dialog" role="dialog" aria-modal="true" aria-labelledby="account-manager-title"><header><div><span class="eyebrow">SV2 ACCOUNT MANAGER</span><h2 id="account-manager-title">${accountManagerSection === "profile" ? t("accounts.profile") : t("accounts.manager")}</h2></div><button class="icon-plain" data-close-account-manager title="${t("accounts.close")}" aria-label="${t("accounts.close")}">×</button></header>${tabs}<div class="account-manager-body">${body}</div></section></div>`;
}
