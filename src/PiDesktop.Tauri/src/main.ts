...
function pathPickerButton(inputId: string, kind: "file" | "audio" | "directory"): string {
  const label = kind === "directory" ? t("system.chooseDirectory") : t("workflowCopy.chooseFile");
  return `<button class="secondary path-picker-button" type="button" data-pick-path="${inputId}" data-pick-path-kind="${kind}" title="${escapeHtml(label)}" aria-label="${escapeHtml(label)}">${icon(kind === "directory" ? "folder" : "file", 16)}</button>`;
}
...
async function pickPathIntoInput(inputId: string, kind: "file" | "audio" | "directory"): Promise<void> {
  ...
  if (!path || page !== selectionPage || !originalInput.isConnected || document.getElementById(inputId) !== originalInput) return;
